...
11. **Implement automated smoke test** based on the Hello World Promise guide to validate end-to-end platform functionality

---

## Fixes Completed (19 December 2025)

The following issues were addressed during this session:

### 1. Created Centralised Configuration File ✅

**File:** `config.env`

All hardcoded values have been extracted to a centralised configuration file including:
- Version numbers (K8S_VERSION, KRATIX_VERSION, CERT_MANAGER_VERSION)
- All URLs (cert-manager, Kratix, MinIO, Flux)
- Namespace names
- API versions
- Timeout values
- Resource names and paths

Scripts now source this file and include fallback defaults if it's missing.

### 2. Pinned Kratix Version ✅

- Changed from `KRATIX_VERSION="latest"` to `KRATIX_VERSION="v0.125.0"`
- Provides reproducible installations

### 3. Fixed Kratix URL Format ✅

- **Before:** `/releases/${KRATIX_VERSION}/download/kratix.yaml` (incorrect)
- **After:** `/releases/download/${KRATIX_VERSION}/kratix.yaml` (correct)

### 4. Fixed MinIO Installation URL ✅

- MinIO manifest now points to `main` branch as it contains fixes for deprecated container images
- The v0.125.0 release used `bitnami/kubectl:1.28.6` which no longer exists on Docker Hub
- The `main` branch uses `ghcr.io/syntasso/kratix-pipeline-utility:v0.0.1`

### 5. Fixed Destination Path Configuration ✅

- **Issue:** Paths were doubled (`worker-cluster/worker-cluster/...`)
- **Cause:** Both `spec.path` and destination name were being used
- **Fix:** Removed redundant `spec.path` from Destination resource
- Flux Kustomisation paths now correctly point to `{destination-name}/dependencies` and `{destination-name}/resources`

### 6. Added Fallback Defaults to All Scripts ✅

All lifecycle scripts now:
- Load configuration from `config.env`
- Display warning message if config file is missing
...
### 7. Updated Kubernetes Version ✅

- Updated from `v1.31.9` to `v1.35.0`

---

## Remaining Items

The following recommendations remain to be addressed in future work:

| Recommendation | Status |
|----------------|--------|
| Add version discovery | Not started |
| Implement checksum validation | Not started |
| Use stable API versions (when available) | Dependent on Kratix graduating APIs |
| Implement service discovery | Not started |
| Add validation steps between phases | Not started |
| Document version compatibility matrix | Not started |
| Add dry-run mode | Not started |
| Improve error handling | Partial (scripts use `set -euo pipefail`) |
| Implement automated smoke test | Not started |
